...
// Função para carregar a fase do arquivo
void loadPhase(int phase, int **board) {
    ...
    for (int i = 0; i < MAX_LINHAS; i++) {
        for (int j = 0; j < MAX_COLUNAS; j++) {
            fscanf(file, "%d", &board[i][j]); // Preenche o tabuleiro com os dados 
        }
    }
...
// Função para colocar a saída em uma posição válida aleatória nas bordas
void positionExit(int **board) {
    ...
    for (int j = 0; j < MAX_COLUNAS; j++) {
        if (board[0][j] == 0) { // Borda superior (linha 0)
    ...
    // Verifica a borda inferior (linha MAX_LINHAS - 1)
    for (int j = 0; j < MAX_COLUNAS; j++) {
    ...
    // Verifica a borda esquerda (coluna 0)
    ...
    // Verifica a borda direita (coluna MAX_COLUNAS - 1)
    for (int i = 1; i < MAX_LINHAS - 1; i++) {  // Evita duplicar os cantos superior e inferior
    ...
    if (count > 0) {
        int randomIndex = rand() % count; // Escolhe uma posição aleatória válida
        int x = validPositions[randomIndex][0];
        int y = validPositions[randomIndex][1];
        board[x][y] = 2; // Marca a posição escolhida como saída
    } else {
